...
def run_model_test():
    ...
    fst_param = {
        'name':f'cost_investment_per_kwh_{bss.name}&{bss.id}',
        'range':range(150,950,80)
    }

    scd_param = {
        'name':f'cost_investment_per_kw_{bss.name}&{bss.id}',
        'range':list(np.arange(150,300,15))
    }
    ...
    modeltest.test_model_two_params(r'temp\test_v2\pricing_test_bss_cost.csv',fst_param,scd_param,*obs_vals)
    
def run_3d_bar_build():
    test = ModelTest('a')
    test.save_3d_chart(r'temp\test_v2\pricing_test_bss_cost.csv',r'temp\test_v2\bar3d',"BSS energy capacity cost(€/kWh)","BSS power capacity cost (€/kW)")
run_3d_bar_build()
# run_model_test()