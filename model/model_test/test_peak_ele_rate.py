...
def run_model_test():
    ...
    fst_param = {
        ...
        'range':range(200,700,50)
    }

    scd_param = {
        'name':f'ele_price_{grid.name}&{grid.id}',
        'range':list(np.arange(0.3,0.5,0.02))
    }
    ...
    modeltest.test_model_two_params(r'temp\test_v2\pricing_test_peak_ele_rate.csv',fst_param,scd_param,*obs_vals)
    
def run_3d_bar_build():
    test = ModelTest('a')
    test.save_3d_chart(r'temp\test_v2\pricing_test_peak_ele_rate.csv',r'temp\test_v2\bar3d','Peak demand rate(€/kW/a)','Electricity price(€/kWh)')
run_3d_bar_build()
# run_model_test()